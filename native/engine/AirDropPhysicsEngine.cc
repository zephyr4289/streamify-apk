...
void AirDropPhysicsEngine::stepRK4(AirDropState& state, const TargetDock& target, float dt) {
    if (state.is_docked) {
        if (state.impact_progress < 1.0f) {
            state.impact_progress = std::min(1.0f, state.impact_progress + (dt / 0.180f));
            float t = state.impact_progress;
            float squashY = 1.0f;
            if (t < 0.25f) {
                squashY = 1.0f - (0.12f * (t / 0.25f));
            } else if (t < 0.60f) {
                squashY = 0.88f + (0.18f * ((t - 0.25f) / 0.35f));
            } else {
                squashY = 1.06f - (0.06f * ((t - 0.60f) / 0.40f));
            }
    ...
    auto computeAccel = [&](float px, float py, float vx, float vy) -> std::pair<float, float> {
        ...
        // Spring stiffness k=180.0, Damping c=24.0
        float fx = 180.0f * dx - 24.0f * vx;
        float fy = 180.0f * dy - 24.0f * vy;

        // Orthogonal lift force creating organic aerodynamic parabolic flight arc
        if (target.initial_dist > 1.0f) {
            float liftMag = 450.0f * std::sin(std::clamp(dist / target.initial_dist, 0.0f, 1.0f) * 3.14159265f);
            fx += (-dy / dist) * liftMag;
            fy += ( dx / dist) * liftMag;
        }
    ...
    state.stretch_parallel = 1.0f + 0.35f * std::tanh(speed / 1200.0f);
    state.stretch_perp = 1.0f / state.stretch_parallel; // Volume conservation: stretch_parallel * stretch_perp = 1.0
    state.rotation_rad = std::atan2(state.vy, state.vx);

    // Dynamic 3D gimbal pitch & roll based on velocity & lateral acceleration
    state.pitch_deg = std::clamp(-state.vy * 0.035f, -15.0f, 15.0f);
    state.roll_deg  = std::clamp( state.vx * 0.035f, -12.0f, 12.0f);
}
