...
#[no_mangle]
pub unsafe extern "C" fn Java_com_streamify_app_data_NativeBridge_nativeBuildPlayerRequest(
    mut env: JNIEnv,
    _class: JClass,
    video_id: JString,
    client_name: JString,
    ...
    let res = catch_unwind(AssertUnwindSafe(|| {
        let vid_str: String = env.get_string(&video_id).map(|s| s.into()).unwrap_or_default();
        let client_str: String = env.get_string(&client_name).map(|s| s.into()).unwrap_or_default();
        let visitor_str: String = env.get_string(&visitor_id).map(|s| s.into()).unwrap_or_default();
        let cookies_str: String = env.get_string(&cookies).map(|s| s.into()).unwrap_or_default();

        let req_spec = crate::json::InnertubeParser::build_player_request(
            &vid_str,
            &client_str,
            &visitor_str,
            sts as u64,
        ...
        let json_out = serde_json::to_string(&req_spec).unwrap_or_default();
